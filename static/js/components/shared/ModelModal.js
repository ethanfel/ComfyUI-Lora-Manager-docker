...
<div class="creator-info" data-username="${modelWithFullData.civitai.creator.username}" data-action="view-creator" title="${translate('modals.model.actions.viewCreatorProfile', {}, 'View Creator Profile')}">
    ...
    const tabPanesContent = modelType === 'loras' ?
        ...
        <div id="community-tab" class="tab-pane">
            <div class="community-tab-content" data-model-hash="${modelWithFullData.sha256 || ''}">
                <div class="community-tab-loading">
                    <i class="fas fa-spinner fa-spin"></i> Loading community creations...
                </div>
            </div>
        </div>` :
        `<div id="showcase-tab" class="tab-pane active">
    ...
    setupTabSwitching({
        onTabChange: async (tab) => {
            ...
            if (tab === 'community') {
                await loadCommunityTab(modelWithFullData.sha256);
            }
        },
    });
...
// ── Community tab loading ────────────────────────────────────────────
async function loadCommunityTab(sha256) {
    const container = document.querySelector('.community-tab-content');
    if (!container || container.dataset.loaded === '1') return;

    if (!sha256) {
        container.innerHTML = '<p style="padding:16px;color:var(--text-secondary);">No model hash available.</p>';
        return;
    }

    try {
        // Try cached data first, then fetch
        let images = window._lmCommunityMap?.[sha256];
        if (!images) {
            const resp = await fetch("/api/lm/community-images/by-hashes", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ hashes: [sha256] }),
            });
            const data = await resp.json();
            images = data.success && data.images ? data.images[sha256] : null;
        }

        if (!images || images.length === 0) {
            container.innerHTML = `
                <div style="text-align:center;padding:40px 16px;color:var(--text-secondary,rgba(255,255,255,0.4));">
                    <i class="fas fa-images" style="font-size:32px;margin-bottom:12px;display:block;"></i>
                    <p>No community creations yet for this model.</p>
                    <p style="font-size:12px;">Use the "Fetch Community Images" button on the Community page.</p>
                </div>`;
        ...
        container.innerHTML = '<p style="padding:16px;color:var(--text-secondary);">Failed to load community images.</p>';
    }
}
