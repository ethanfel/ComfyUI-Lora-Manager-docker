...
describe('Showcase gallery', () => {
  ...
  it('escapes workflow filenames rendered into button attributes', async () => {
    const { renderShowcaseContent } = await import(SHOWCASE_MODULE);
    const filename = 'image_0.\" onmouseover=\"alert(1).png';
    const localFiles = [{
      name: filename,
    ...
    const host = document.createElement('div');
    host.innerHTML = renderShowcaseContent([IMAGES[0]], localFiles, '', true);

    const button = host.querySelector('.workflow-download-btn');
    expect(button).toBeTruthy();
    expect(button.dataset.filename).toBe(filename);
